...
export default async function handler(req, res) {
  ...
  const payload = JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 2000, messages: body?.messages || [] })
  return new Promise((resolve) => {
    ...
    const request = https.request(options, (response) => {
      ...
      response.on('end', () => {
        try { res.status(response.statusCode).json(JSON.parse(data)) } catch { res.status(500).json({ error: 'Parse error' }) }
        resolve()
      })
    })
